#!/usr/bin/env bash
# product-shots `render` skill - dependency installer.
#
...
if python3 -c 'import requests' 2>/dev/null; then
    ...
    echo "[render] requests already installed: $REQV"
else
    echo "[render] installing requests..."
...
if python3 -c 'from PIL import Image' 2>/dev/null; then
    ...
    echo "[render] Pillow already installed: $PILV"
else
    echo "[render] installing Pillow..."
    python3 -m pip install --quiet 'Pillow>=10.0'
    need_pip_install=true
fi

# --- API key + base URL reminders ---
if [ -z "${OMNIMAAS_API_KEY:-}" ] && [ -z "${RENDER_API_KEY:-}" ] && [ -z "${CANVASFLOW_IMAGEGEN_API_KEY:-}" ] && [ ! -f "$HOME/.product_shots_render_api_key" ] && [ ! -f "$HOME/.canvasflow_imagegen_api_key" ]; then
    echo ""
    ...
    echo "    echo 'your-token' > ~/.product_shots_render_api_key && chmod 600 ~/.product_shots_render_api_key"
fi

if [ -z "${OMNIMAAS_BASE_URL:-}" ] && [ -z "${RENDER_BASE_URL:-}" ] && [ -z "${CANVASFLOW_IMAGEGEN_BASE_URL:-}" ] && [ -z "${OMNIMAAS_API_KEY:-}" ]; then
    echo ""
    ...
    echo "    export RENDER_BASE_URL='https://your-gateway/v1'"
fi

echo ""
if $need_pip_install; then
    echo "[render] setup complete. Dependencies installed."
else
    echo "[render] setup complete. All dependencies already present."
fi
